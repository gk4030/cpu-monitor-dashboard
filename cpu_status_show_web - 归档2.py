...
THRESHOLD = 80.0            # CPU 告警阈值 (%)
HIGH_LOAD = 60.0            # 高负载阈值 (%)
LOW_LOAD = 20.0             # 空闲阈值 (%)
ALL = "全部"

# 默认 LLM 配置（首次运行会生成配置文件）
DEFAULT_LLM_CONFIG = {
    "enabled": False,
...
<head>
    ...
    <style>
        ...
        .modal { background: #fff; padding: 24px; border-radius: 8px; width: 450px; max-width: 90%; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
    ...
    <div class="modal-overlay" id="configModal">
        <div class="modal">
            ...
            <div class="form-group">
                ...
                <div class="hint">支持 OpenAI、DeepSeek、Ollama 等兼容接口</div>
            </div>
            <div class="form-group">
                <label>API Key</label>
            ...
            <div class="form-group">
                <label>模型名称</label>
                <input type="text" id="cfgModel" placeholder="gpt-3.5-turbo / deepseek-chat">
            </div>
            <div class="form-group">
                <label>超时时间 (秒)</label>
    ...
    <script>
        ...
        async function updateAll() {
            ...
            try {
                ...
                content.innerHTML = `<div class="analysis-empty">⚠️ 数据加载失败: ${e.message}。请检查网络连接或AI配置。</div>`;
            }
        }
        ...
        function renderAnalysis(data) {
            ...
            if (data.use_llm && data.llm_report) {
                ...
                const mdHtml = marked.parse(data.llm_report);
                html += `
                    <div class="llm-report">
                        <h4>🤖 AI 专家诊断报告</h4>
                        <div class="llm-text">${mdHtml}</div>
                    </div>
                `;
            }
